...
Conjunto de procedimentos cujo objetivo é a coleta de dados para compor uma amostra representativa de uma população.
...
```{r}
#| label: fig-vieses
#| fig-cap: "Disparos de armas de fogo."
#| fig-subcap: ["Arma não viesada.", "Arma viesada"]
#| code-fold: true
#| layout-ncol: 2
sample <- data.frame(x = runif(n = 20, min = -2, max = 2),
                      y = runif(n = 20, min = -2, max = 2)
                      )
...
sample <- data.frame(x = runif(n = 20, min = 1, max = 5),
                      y = runif(n = 20, min = -2, max = 2)
                      )
g2 <- ggplot()+
  geom_circle(data = data.frame(x0 = rep(0, 5),
...
forecast::autoplot(AirPassengers)+ggplot2::labs(x = "Ano", y = "Número de passageiros")
```
...
![Amostragem aleatória estratificada.](amostragem_estratificada.jpg){#fig-amostragem_estratificada width=80%}
...
Delineamento experimental cujo objetivo é obter um modelo estatístico onde analisamos a influência de duas variáveis independentes sobre uma dependente. Para isso, os tratamentos são planejados de maneira a englobar dois fatores simultâneos.

#### Principais características

* Obtenção de modelos de regressão;

* Otimização de experimentos;

* Permite apenas 2 fatores;

* Os fatores devem ser quantitativos contínuos.

A @fig-analise_superficie apresenta a vazão de gás hidrogênio (variável dependente) em um processo de eletrólise contendo água e NaOH [@Luiz2025]. Este resultado foi obtido utilizando o método fatorial n3^k^, onde n representa a quantidade de réplicas dos tratamentos e k o número de fatores a serem analisados como variáveis independentes. Neste caso temos dois fatores, tensão e concentração de NaOH, então podemos dizer que k = 2.
